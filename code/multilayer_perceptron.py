...
class MultilayerPerceptron:
    def __init__(self, input_size: int, output_size: int, number_of_layers: int, number_of_neurons_in_layers: list,
                 activ_functions: list):
        self.in_size = input_size
        self.out_size = output_size
        self.n_layers = number_of_layers
        self.network = self.initialize_perceptron(number_of_neurons_in_layers, activ_functions)

    def initialize_perceptron(self, number_of_neurons_in_layers, activations):
        """
        function that initializes all the layers and all the neurons
        """
        network = []
        ...
        for i in range(0, self.n_layers):
            ...
            for j in range(0, num_neurons[i]):
                neuron = Neuron(n_inputs_to_neuron, activations[i])
                layer.append(neuron)
    ...
    def train_network(self, x_data, y_data, n_epochs, n_batches, beta):
        """
        Basic version of train function.
        """
        ...
        for i in range(0, n_epochs):
            ...
            for j in range(0, n_batches):
                ...
                self.backpropagation_of_error(x_batch, y_batch, beta)
            print("")
    ...
    def train_network_get_learning_curve(self, x_data, y_data, x_valid, y_valid, n_epochs, n_batches, beta):
        ...
        train_quality = self.calculate_loss_function(y_data, np.reshape(self.get_multiple_predictions(x_data),
                                                                        np.shape(y_data)))
        validation_quality = self.calculate_loss_function(y_valid, np.reshape(self.get_multiple_predictions(x_valid),
                                                                              np.shape(y_valid)))
        ...
        for i in range(0, n_epochs):
            ...
            train_quality = self.calculate_loss_function(y_data, np.reshape(self.get_multiple_predictions(x_data),
                                                                            np.shape(y_data)))
            validation_quality = self.calculate_loss_function(y_valid,
                                                              np.reshape(self.get_multiple_predictions(x_valid),
                                                                         np.shape(y_valid)))
            epoch_data_train.append(np.nanmean(train_quality, dtype='float32'))
            epoch_data_test.append(np.nanmean(validation_quality, dtype='float32'))
            print("")
    ...
    def backpropagation_of_error(self, x, y, beta):
        """
        ...
        """
        y_predictions = self.get_multiple_predictions(x)
        error_vec = self.calculate_loss_function_differential(y, y_predictions)
        weight_diffs = []
        bias_diffs = []
        ...
        for k in range(0, len(x)):
            ...
            delta, b_diff, w_diff = self.get_weight_diff(input_layer, curr_layer, deltas=None, error_calc=error,
                                                         is_output=True)
            ...
            for i in range(len(self.network) - 2, 0, -1):
                ...
                delta, b_diff, w_diff = self.get_weight_diff(input_layer, curr_layer, deltas=delta, error_calc=error,
                                                             is_output=False)
                weight_diffs[k].append(w_diff)
                bias_diffs[k].append(b_diff)

            delta, b_diff, w_diff = self.get_weight_diff(x_current, self.network[0], deltas=delta, error_calc=error,
                                                         is_output=False)
            weight_diffs[k].append(w_diff)
            bias_diffs[k].append(b_diff)
        for layer_id in range(0, len(weight_diffs[0])):
            for neuron_id in range(0, len(weight_diffs[0][layer_id])):
                bias_for_neuron = [bias_diffs[sample_id][layer_id][neuron_id] for sample_id in
                                   range(0, len(bias_diffs))]
                weights_for_neuron = [weight_diffs[sample_id][layer_id][neuron_id] for sample_id in
                                      range(0, len(weight_diffs))]
                ...
                weights_diff_for_neuron = [np.nanmean(weights_for_neuron_trans[p], dtype='float32') for p in
                                           range(0, len(weights_for_neuron_trans))]
    ...
    @staticmethod
    def get_weight_diff(input_layer, curr_layer, deltas=None, error_calc=None, is_output=False):
        """
        ...
        if not isinstance(input_layer[0], numbers.Number):
            if is_output:
                for k in range(0, len(curr_layer)):
                    single_delta = calc_derivative(curr_layer[k].activation_function,
                                                   curr_layer[k].output_before_activation) * error_calc[k]
            ...
            for j in range(0, len(input_layer)):
                single_derivative = calc_derivative(input_layer[j].activation_function,
                                                    input_layer[j].output_before_activation)
                sum_of_deltas = 0
                for k in range(0, len(curr_layer)):
    ...
    @staticmethod
    def calculate_loss_function(correct_output, predicted_output):
        """
        loss = (y_predicted - y_correct)^2
        """
        return np.power(np.add(np.array(predicted_output), -np.array(correct_output)), 2)

    @staticmethod
    def calculate_loss_function_differential(correct_output, predicted_output):
        """
        diff_loss = 2 * (y_predicted - y_correct)
        """
