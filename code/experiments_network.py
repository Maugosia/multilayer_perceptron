...
def experiment_and():
    ...
    number_of_layers = 2
    neuron_quantities = [3]
    activation_functions = [tanh_act, sigmoid_act]
    ...
    net_mlp = MultilayerPerceptron(input_size, output_size, number_of_layers, neuron_quantities, activation_functions)
...
def experiment_house_price_prediction():
    ...
    print("Shape of validation data X {}, validation data y {}".format(np.shape(valid_x), np.shape(valid_y)))
...
def experiment_mnist():
    ...
    # -------------------------------INITIALIZE AND TRAIN NETWORK---------------------------------------------------
    net_mlp = MultilayerPerceptron(input_size, output_size, number_of_layers, neuron_quantities, activation_funs)
    train, val = net_mlp.train_network_get_learning_curve(train_x, train_y, validation_x, validation_y,
                                                          number_of_epochs, number_of_batches, beta)

    # --------------------------------------DISPLAY RESULTS-----------------------------------------------------------
    plot_learning_data(train, val, "Funkcja straty - MNIST")

    pred = net_mlp.get_multiple_predictions_numeric(train_x)
    correct = []
    for element in train_y:
        correct.append(np.argmax(element))
    plot_confusion_matrix(pred, correct, "Macierz pomyłek zbioru treningowego MNIST")

    pred = net_mlp.get_multiple_predictions_numeric(validation_x)
    correct = []
    for element in validation_y:
        correct.append(np.argmax(element))
    plot_confusion_matrix(pred, correct, "Macierz pomyłek zbioru walidcyjnego MNIST")

    pred = net_mlp.get_multiple_predictions_numeric(test_x)
    correct = []
    for element in test_y:
        correct.append(np.argmax(element))
    plot_confusion_matrix(pred, correct, "Macierz pomyłek zbioru testowego MNIST")
